...
int main(void) {
	...
	PSTUDENT current = NULL;

	PSTUDENT x1 = create_student(&current, 1358329581, "ExampleFirst", "ExampleMiddle", "ExampleLast");
	PSTUDENT x2 = create_student(&current, 1493850039, "Josh", "Middleton", "Taylor");
	PSTUDENT x3 = create_student(&current, 1950285749, "Isaac", "", "Placen");
	PSTUDENT x4 = create_student(&current, 1945274921, "Yusif", "", "Appleton");

	print_students(current);
	destroyStudentList(&current);
